...
class BatchAIVerifier:
    ...
    def _build_instructions(self) -> str:
        return """You are an audiobook metadata expert. You will receive a batch of audiobooks
...
For audiobooks that are perfect, you can omit them (assume unchanged)."""
    ...
    def verify_batch(
        ...
        # Build prompt with all audiobooks
        items = []
        for i, (source, metadata, quality) in enumerate(audiobooks):
            item = {
                "index": i,
...
Omit audiobooks that are already correct."""
        ...
        return {v.index: v for v in result.output.audiobooks}
